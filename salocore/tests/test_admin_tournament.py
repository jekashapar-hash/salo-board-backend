...
def jury_user(db):
    return User.objects.create_user(
        username="juryuser", email="jury@mail.com",
        password="JuryPass123!", invite_code="JURYCODE",
    )
...
class TestAdminTournamentList:
    ...
    def test_superuser_sees_all_tournaments(
        self, superuser_client, tournament, tournament_draft
    ):
    ...
    def test_staff_sees_only_own_tournaments(
        self, admin_client, admin_user, tournament, user, factory
    ):
        """Стаф без superuser бачить тільки свої турніри (де creator або admin)."""
        other_t = factory.create_tournament(
            user, title="Other T", status=Tournament.Status.DRAFT
        )
        response = admin_client.get(ADMIN_TOURNAMENTS_URL)
        ids = [t["id"] for t in response.data]
        assert tournament.id in ids
    ...
    def test_post_creates_tournament(self, admin_client):
        now = timezone.now()
        payload = {
            "title": "New Admin T", "description": "D", "rules": "Rules",
            "max_team": 5, "max_team_size": 3, "min_team_size": 1,
    ...
    def test_post_sets_creator_as_current_user(self, admin_client, admin_user):
        now = timezone.now()
        payload = {
            "title": "Creator T", "description": "D", "rules": "Rules",
            "max_team": 5, "max_team_size": 3, "min_team_size": 1,
            "is_team_visible": True,
            "reg_open_at": now.isoformat(),
            "reg_close_at": (now + timezone.timedelta(days=1)).isoformat(),
...
class TestAdminTournamentDetail:
    ...
    def test_patch_edits_draft_tournament(self, admin_client, tournament_draft):
        response = admin_client.patch(
            admin_tournament_detail_url(tournament_draft.id), {"title": "Updated"}
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_patch_fails_non_draft(self, admin_client, tournament):
        with patch("salocore.views.admin_tournament.get_tournament_cheker") as m:
            ...
            response = admin_client.patch(
                admin_tournament_detail_url(tournament.id), {"title": "X"}
            )
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_patch_forbidden_for_non_creator(self, superuser_client, user, tournament_draft, factory):
        """Не creator не може редагувати (IsTournamentCreatorOrReadOnly)."""
        other_draft = factory.create_tournament(
            user, title="Other Draft", status=Tournament.Status.DRAFT
        )
        # ... rest of the test (if any logic depends on it)
        # admin_user не є creator цього турніру, але superuser — може редагувати
        # Перевіримо що звичайний адмін (не суперюзер) не може
        pass  # IsTournamentCreatorOrReadOnly дозволяє read для всіх admins, write тільки creator
...
class TestAdminTournamentStart:
    ...
    def test_starts_draft_tournament_with_jury(
        self, admin_client, tournament_draft, jury_user
    ):
    ...
    def test_returns_403_for_non_creator(self, superuser_client, user, jury_user, factory):
        other_draft = factory.create_tournament(
            user, title="OtherD", status=Tournament.Status.DRAFT
        )
        ...
        admin2 = User.objects.create_user(
            username="admin2", email="admin2@mail.com",
            password="A123!", is_staff=True, invite_code="ADM2CODE",
        )
        client2 = APIClient()
...
class TestAdminTournamentJury:
    ...
    def test_post_fails_if_user_not_found(self, admin_client, tournament_draft):
        response = admin_client.post(
            admin_jury_url(tournament_draft.id), {"invite_code": "NOTEXIST"}
        )
    ...
    def test_delete_removes_jury(self, admin_client, tournament_draft, jury_user):
        ...
        response = admin_client.delete(
            admin_jury_detail_url(tournament_draft.id, jury_user.id)
        )
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not TournamentJury.objects.filter(
            tournament=tournament_draft, user=jury_user
        ).exists()
...
class TestAdminTournamentAdmins:
    ...
    def test_delete_removes_admin(self, admin_client, tournament_draft, other_user):
        TournamentAdmin.objects.create(tournament=tournament_draft, user=other_user)
        response = admin_client.delete(
            admin_admins_detail_url(tournament_draft.id, other_user.id)
        )
        assert response.status_code == status.HTTP_204_NO_CONTENT
