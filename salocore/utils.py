...
def check_and_update_round_deadlines(round_obj):
    ...
    if timezone.now() > round_obj.deadline:
        ...
        teams_without_subs = Team.objects.filter(tournament=round_obj.tournament).exclude(id__in=teams_with_submissions)
...
def check_tournament_deadlines(tournament):
    ...
    """
    active_rounds = Round.objects.filter(tournament=tournament, status=Round.Status.ACTIVE)
